...
interface DropDownHookParams {
  (items : string[], init : string, setFn : (option : string)=>void) : {isDropDownShow : boolean, selectedValue : string, handleDropDown : ()=>void, handleCurrntIndex : (e :React.MouseEvent<HTMLButtonElement, MouseEvent>)=>void }
}
...
export const useDropDown : DropDownHookParams = (items, init, setFn) => {
  ...
  const handleCurrntIndex = (e :React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    const {target} = e
    setIsDropDownShow(prev => !prev);
