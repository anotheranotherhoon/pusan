...
import type {
  RestaurantInfoType,
  RestaurantInitialStateType,
} from "../types/restaurantType";

const initialState: RestaurantInitialStateType = {
  status: null,
  restaurantList: [],
  filteredRestaurant: [],
  ...
  reducers: {
    filterRestaurant: (state, action) => {
      if (action.payload.option === "지역을 선택하세요") {
        ...
        state.filteredRestaurant = action.payload.restaurantList.filter(
          (el: RestaurantInfoType) => el.GUGUN_NM === action.payload.option
        );
      }
    },
  },
