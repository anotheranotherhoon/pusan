...
const DropDown = ({ handleFilter, init }: DropDownProps) => {
  const { isDropDownShow, selectedValue, handleDropDown, handleCurrntIndex } =
    useDropDown(VILLAGE_FILLTER_OPTION, init, handleFilter);
  return (
    <DropDownWrapper>
...
export const DropDownBtn = styled.button`
  ...
  background: #ffffff;
  border: ${(props) =>
    props.theme.theme === "light" ? "1px solid #77af9c" : "1px solid grey"};
  color: ${(props) => (props.theme.theme === "light" ? "black" : "grey")};
  background-color: ${(props) =>
    props.theme.theme === "light" ? "white" : "darkslategrey"};
  border-radius: 10px;
  width: 200px;
  padding: 12px 13px;
  :focus {
